...
describe('TypeScript runtime',()=>{
 ...
 it('rejects a run-directory rename and symlink race before accepting external output',async()=>{
   const f=await fixture(''); const externalDir=await mkdtemp(join(tmpdir(),'awgpt-external-')); const external=join(externalDir,'output.md');
   await writeFile(f.exe, `const fs=require('fs'); const a=process.argv; if(a.includes('--version')) { console.log('0.17.1'); process.exit(0); } const i=a.indexOf('--write-output'); const p=a[i+1]; const d=require('path').dirname(p); fs.renameSync(d,d+'.moved'); fs.symlinkSync(${JSON.stringify(externalDir)},d); fs.writeFileSync(p,'TASK_OUTCOME: EXECUTED\\n'); process.exit(0);`);
   await expect(runOracle({projectRoot:f.root,missionPath:f.mission,oracleCommand:[process.execPath,f.exe]})).rejects.toThrow('RUN_DIRECTORY_CHANGED');

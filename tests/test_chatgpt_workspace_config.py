...
def test_workspace_app_name_defaults_and_supports_host_config(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    module = load()
    monkeypatch.setenv("AGENT_WEB_GPT_HOME", str(tmp_path))
    monkeypatch.setenv("CODEX_HOME", str(tmp_path / "legacy"))
    monkeypatch.delenv("AGENT_WEB_GPT_APP_NAME", raising=False)
    ...
    monkeypatch.setenv("CODEX_CHATGPT_APP_NAME", "legacy-temporary")
    monkeypatch.setenv("AGENT_WEB_GPT_APP_NAME", "temporary")

...
import { chmodSync, mkdtempSync, readFileSync, rmSync, existsSync, realpathSync, writeFileSync } from 'node:fs';
...
try {
 ...
 server=createServer((q,r)=>{let b='';q.on('data',c=>b+=c);q.on('end',()=>{const n=JSON.parse(b).params.name;if(n==='open_workspace'){counters.open++;r.setHeader('content-type','application/json'); r.end(JSON.stringify({result:{allowedRoots:[JSON.parse(b).params.arguments.root]}}));}else{counters.ls++;r.setHeader('content-type','application/json'); r.end(JSON.stringify({result:{entries:[]}}));}})}); await listen(server); const ep=`http://127.0.0.1:${server.address().port}/mcp`;
 const fake=join(temp,'oracle.mjs'), cf=join(temp,'oracle-counter'); writeFileSync(fake,"#!/usr/bin/env node\nimport {appendFileSync,writeFileSync} from 'node:fs'; if(process.argv.includes('--version')) { console.log('0.17.1'); process.exit(0); } appendFileSync(process.env.E2E_COUNTER,JSON.stringify(process.argv.slice(2))+'\\n'); const i=process.argv.indexOf('--write-output'); if(i>=0) writeFileSync(process.argv[i+1],'TASK_OUTCOME: EXECUTED\\n'); console.log('TASK_OUTCOME: EXECUTED');"); chmodSync(fake,0o755); const env={...process.env,E2E_COUNTER:cf};
 const pos=JSON.parse(await run(['run','--project-root',project,'--mission',mission,'--devspace-url',ep,'--oracle-command',fake],{env})); if(pos.state.task_outcome!=='EXECUTED'||counters.open!==1||counters.ls!==1||!existsSync(cf)) throw Error('positive E2E failed '+JSON.stringify({pos,counters,exists:existsSync(cf)}));
 const project2=join(temp,'project2'); execFileSync('mkdir',['-p',project2]); const mission2=join(project2,'mission.md'); writeFileSync(mission2,'# smoke\n'); const bad=createServer((q,r)=>{q.resume();q.on('end',()=>r.end(JSON.stringify({result:{allowedRoots:[]}})));}); await listen(bad); const nep=`http://127.0.0.1:${bad.address().port}/mcp`; const neg=JSON.parse(await run(['run','--project-root',project2,'--mission',mission2,'--devspace-url',nep,'--oracle-command',fake],{env})); bad.close(); if(neg.state.transport_status!=='failed') throw Error('negative gate failed');

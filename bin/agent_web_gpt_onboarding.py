...
from agent_web_gpt_paths import resolve_app_name, resolve_home
...
def onboarding_plan(
    *,
    ...
    return {
        "schema": "agent-web-gpt.onboarding-plan/v1",
...
def readiness_status(
    *,
    ...
    codex_home = resolve_home(codex_home)
    ...
    return {
        "schema": "agent-web-gpt.onboarding-status/v1",
...
def configure_app_name(*, agent_home: Path | None = None, codex_home: Path | None = None, app_name: str = APP_NAME) -> Path:
    app_name = normalize_app_name(app_name)
    root = resolve_home(agent_home or codex_home)
...
def _parser() -> argparse.ArgumentParser:
    ...
    configure.add_argument("--agent-home", type=Path, dest="agent_home")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "plan":
            ...
            path = configure_app_name(agent_home=args.agent_home, app_name=args.app_name)

...
def compat_state_root() -> Path:
    return state_root("AGENT_WEB_GPT_DEVSPACE_COMPAT_STATE_ROOT", "CODEX_DEVSPACE_COMPAT_STATE_ROOT", "devspace-compat", SUPPORTED_VERSION)
...
def ensure_devspace_compatibility(
    *,
    ...
    backup = backup_root or (resolve_home() / "state" / "devspace-compat-backups" / SUPPORTED_VERSION)
    changed: list[str] = []
    already: list[str] = []
    for root in roots:

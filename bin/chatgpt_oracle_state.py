...
import json
import os
from agent_web_gpt_paths import resolve_home, state_root
import re
import shutil
import subprocess
import tempfile
...
ORACLE_AUTH_EXPIRED_PRE_SUBMIT_RE = re.compile(
    r"(?:Session appears logged out|redirected to (?:auth/login|login\.openai\.com)|"
    r"Authentication required|Please log in to continue|Log in to ChatGPT|"
    r"No valid session cookies found|auth_expired|session_unauthenticated)",
    re.IGNORECASE,
)
ORACLE_CLOUDFLARE_CHALLENGE_PRE_SUBMIT_RE = re.compile(
    r"(?:Cloudflare challenge detected|Verify you are human|Just a moment\.\.\.|"
    r"challenges\.cloudflare\.com|cf-turnstile|Checking your browser before accessing|"
    r"Attention Required! \| Cloudflare)",
    re.IGNORECASE,
)
ORACLE_PRO_QUOTA_EXHAUSTED_PRE_SUBMIT_RE = re.compile(
    r"(?:You'?ve reached (?:the|your) (?:Pro|GPT-5\.6 Sol) limit|"
    r"Pro limit reached|usage limit for (?:Pro|GPT-5\.6 Sol)|"
    r"Your limit for Pro has been reached|Try again (?:after|at) [0-9]|"
    r"quota exhausted|pro_quota_reached|pro_limit_exceeded)",
    re.IGNORECASE,
)
...
ATTACHMENT_TRANSPORTS = frozenset(("pro-attachment-only", "deep-research-attachment-only", "attachment-only"))
...
def oracle_state_root() -> Path:
    return state_root("AGENT_WEB_GPT_ORACLE_STATE_ROOT", "CODEX_ORACLE_STATE_ROOT", "chatgpt-oracle")
...
def load_manifest(path: Path, *, platform_name: str | None = None) -> OracleConfig:
    ...
    if transport not in {"devspace", "deep-research-attachment-only", *PRO_TRANSPORTS}:
        raise OracleStateError(
            "TRANSPORT_INVALID",
            "transport must be devspace, deep-research-attachment-only, pro-devspace, pro-attachment-only, or legacy pro-devspace-readonly",
        )
    ...
    if is_devspace_transport(transport):
        ...
        if app_name_raw:
            code = "PRO_APP_FORBIDDEN" if is_pro_transport(transport) else "APP_FORBIDDEN"
            raise OracleStateError(code, "attachment-only runs must not name an app")
        app_name = None
        raw_attachments = payload.get("attachments")
        if not isinstance(raw_attachments, list) or not raw_attachments:
            code = "PRO_ATTACHMENTS_REQUIRED" if is_pro_transport(transport) else "ATTACHMENTS_REQUIRED"
            raise OracleStateError(code, "attachment runs require one or more exact attachment files")
        attachments = tuple(
        ...
        if len(set(attachments)) != len(attachments):
            code = "PRO_ATTACHMENTS_DUPLICATE" if is_pro_transport(transport) else "ATTACHMENTS_DUPLICATE"
            raise OracleStateError(code, "attachment paths must be unique")
        if mission_path not in attachments:
            code = "PRO_MISSION_ATTACHMENT_REQUIRED" if is_pro_transport(transport) else "MISSION_ATTACHMENT_REQUIRED"
            raise OracleStateError(code, "mission_path must be one of the attachments")
    ...
    if is_attachment_transport(transport) and task_outcome_contract != "legacy":
        code = "PRO_TASK_OUTCOME_CONTRACT_FORBIDDEN" if is_pro_transport(transport) else "ATTACHMENT_TASK_OUTCOME_CONTRACT_FORBIDDEN"
        raise OracleStateError(
            code,
            "attachment-only output is not wrapped in the DevSpace task outcome contract",
        )
...
def composer_prompt(config: OracleConfig, mission_path: Path | None = None) -> str:
    if is_attachment_transport(config.transport):
        if config.research == "deep":
            return "Read the attached mission file and all attached files/code, then complete the research task."
...
def proven_pre_submit_auth_expired(state_path: Path) -> dict[str, Any] | None:
    """Prove ChatGPT session expired or login page was encountered before prompt send."""
    state = load_state(state_path)
    if str(state.get("session_authority") or "") not in {"pre_submit", "submitted_unknown"}:
        return None
    if state.get("terminal_harvested") is True or _state_has_conversation_url(state):
    ...
    if CHATGPT_CONVERSATION_URL_RE.search(combined):
        return None
    if ORACLE_AUTH_EXPIRED_PRE_SUBMIT_RE.search(combined) is None:
        return None
    oracle = state.get("oracle") if isinstance(state.get("oracle"), dict) else {}
    locator = str(oracle.get("session_locator") or oracle.get("slug") or "").strip()
    if not locator:
    ...
    return {
        "schema": "codex.chatgpt.oracle-pre-submit-ui-failure/v1",
        "code": "ORACLE_AUTH_EXPIRED_PRE_SUBMIT_FAILED",
        "oracle_locator": locator,
        "stdout_sha256": hashlib.sha256(stdout_bytes).hexdigest(),
        "stderr_sha256": hashlib.sha256(stderr_bytes).hexdigest(),
        "output_absent": True,
        "conversation_url_absent": True,
        "failure_reason": "oracle-auth-expired-or-login-required",
    }


def proven_pre_submit_cloudflare_challenge(state_path: Path) -> dict[str, Any] | None:
    """Prove a Cloudflare interstitial/turnstile challenge stopped Oracle before prompt send."""
    state = load_state(state_path)
    if str(state.get("session_authority") or "") not in {"pre_submit", "submitted_unknown"}:
        return None
    if state.get("terminal_harvested") is True or _state_has_conversation_url(state):
    ...
    if CHATGPT_CONVERSATION_URL_RE.search(combined):
        return None
    if ORACLE_CLOUDFLARE_CHALLENGE_PRE_SUBMIT_RE.search(combined) is None:
        return None
    oracle = state.get("oracle") if isinstance(state.get("oracle"), dict) else {}
    locator = str(oracle.get("session_locator") or oracle.get("slug") or "").strip()
    if not locator:
    ...
    return {
        "schema": "codex.chatgpt.oracle-pre-submit-ui-failure/v1",
        "code": "ORACLE_CLOUDFLARE_CHALLENGE_PRE_SUBMIT_FAILED",
        "oracle_locator": locator,
        "stdout_sha256": hashlib.sha256(stdout_bytes).hexdigest(),
        "stderr_sha256": hashlib.sha256(stderr_bytes).hexdigest(),
        "output_absent": True,
        "conversation_url_absent": True,
        "failure_reason": "oracle-cloudflare-challenge-interstitial",
    }


def proven_pre_submit_pro_quota_exhausted(state_path: Path) -> dict[str, Any] | None:
    """Prove Pro usage limit was reached on the account before prompt send."""
    state = load_state(state_path)
    if str(state.get("session_authority") or "") not in {"pre_submit", "submitted_unknown"}:
        return None
    if state.get("terminal_harvested") is True or _state_has_conversation_url(state):
    ...
    if CHATGPT_CONVERSATION_URL_RE.search(combined):
        return None
    if ORACLE_PRO_QUOTA_EXHAUSTED_PRE_SUBMIT_RE.search(combined) is None:
        return None
    oracle = state.get("oracle") if isinstance(state.get("oracle"), dict) else {}
    locator = str(oracle.get("session_locator") or oracle.get("slug") or "").strip()
    if not locator:
    ...
    return {
        "schema": "codex.chatgpt.oracle-pre-submit-ui-failure/v1",
        "code": "ORACLE_PRO_QUOTA_EXHAUSTED_PRE_SUBMIT_FAILED",
        "oracle_locator": locator,
        "stdout_sha256": hashlib.sha256(stdout_bytes).hexdigest(),
        "stderr_sha256": hashlib.sha256(stderr_bytes).hexdigest(),
        "output_absent": True,
        "conversation_url_absent": True,
        "failure_reason": "oracle-pro-quota-limit-reached",
    }
...
def proven_pre_submit_failure(state_path: Path) -> dict[str, Any] | None:
    return (
        ...
        or proven_pre_submit_auth_expired(state_path)
        or proven_pre_submit_cloudflare_challenge(state_path)
        or proven_pre_submit_pro_quota_exhausted(state_path)
...
def settle_proven_pre_submit_failure(state_path: Path) -> dict[str, Any] | None:
    ...
    if evidence is None:
        evidence = proven_pre_submit_auth_expired(state_path)
    if evidence is None:
        evidence = proven_pre_submit_cloudflare_challenge(state_path)
    if evidence is None:
        evidence = proven_pre_submit_pro_quota_exhausted(state_path)
    if evidence is None:
    ...
    payload.update({
        ...
        "task_outcome": (
            ...
            if evidence["code"] in {
                ...
                "ORACLE_AUTH_EXPIRED_PRE_SUBMIT_FAILED",
                "ORACLE_CLOUDFLARE_CHALLENGE_PRE_SUBMIT_FAILED",
                "ORACLE_PRO_QUOTA_EXHAUSTED_PRE_SUBMIT_FAILED",
            }
            else "pending"
        ),
        "task_outcome_reason": (
            ...
            else "oracle-auth-expired-pre-submit"
            if evidence["code"] == "ORACLE_AUTH_EXPIRED_PRE_SUBMIT_FAILED"
            else "oracle-cloudflare-challenge-pre-submit"
            if evidence["code"] == "ORACLE_CLOUDFLARE_CHALLENGE_PRE_SUBMIT_FAILED"
            else "oracle-pro-quota-exhausted-pre-submit"
            if evidence["code"] == "ORACLE_PRO_QUOTA_EXHAUSTED_PRE_SUBMIT_FAILED"
...
def classify_task_outcome(path: Path, *, contract: str, transport: str) -> str:
    ...
    if len(marker_lines) == 1:
        ...
        if _only_bounded_reference_definitions(text.splitlines()[index + 1 :]):

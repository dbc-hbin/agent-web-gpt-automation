...
def _pro_stage_mission(
    ...
    protocol = (
        ...
        f"workflow_id={workflow_id}\nstage=pro\nstage_index={index}\n"
        f"attempt_id={attempt_id}\ninput_mission_sha256={input_sha}\n"
        "Produce your complete, in-depth architectural design, implementation plan, and recommendations. "
        "You may format your response freely using clear Markdown.\n"
    )
...
def _load_pro_envelope(
    ...
    *,
    ...
    stripped = text.strip()
    if not stripped:
        raise WorkflowError("Pro Oracle output is empty")
    ...
    try:
        envelope = json.loads(cleaned, object_pairs_hook=reject_duplicate_keys)
        if isinstance(envelope, dict) and "schema" in envelope:
            return envelope, None
    except json.JSONDecodeError as exc:
        if "schema" in stripped and PRO_OUTPUT_SCHEMA in stripped:
    ...
    return {
        "schema": PRO_OUTPUT_SCHEMA,
        "workflow_id": workflow_id or "a" * 32,
        "stage": "pro",
        "attempt_id": attempt_id or "b" * 32,
        "input_mission_sha256": input_sha or "c" * 64,
        "status": "PASS",
        "output_text": text,
        "next_stage": "review",
        "next_mission_text": (
            "# Review and Implementation Mission from Pro Design\n\n"
            f"## Pro Design Output\n\n{text}\n\n"
            "## Task\nReview the Pro design above, repair/finalize the plan if needed, and execute implementation."
        ),
        "ready_for_next": True,
        "blocker": "",
    }, {
        "schema": PRO_OUTPUT_RECOVERY_SCHEMA,
        "method": "freeform-markdown-design-envelope/v1",
        "source_output_sha256": sha(output_path),
    }
...
def _materialize_pro_receipt(
    ...
    *,
    ...
    envelope, recovery = _load_pro_envelope(
        output_path,
        workflow_id=workflow_id,

...
For non-Pro regular `GPT` runs through Oracle+DevSpace, preserve the selected operation mode:
...
- New non-Pro direct, plan, review, edit, orchestrator, comprehensive, and Web Multi work uses Oracle plus the manually registered `DevSpace` workspace. The composer receives only `@DevSpace` and the absolute UTF-8 mission path. Deep Research operates without DevSpace: it passes the mission and evidence/code packet (or ZIP) as attachments. Summarize and package only relevant source code and analysis documents (under 1 MiB via `bin/chatgpt_context_packer.py`), excluding `.git`, `node_modules`, `.venv`, and binaries.
...
For a new DevSpace project task, a failed or unavailable endpoint blocks submission and routes only to `chatgpt-workspace-setup` diagnosis. It never authorizes Playwright, in-app Browser, or @chrome fallback. `pro-attachment` requires its exact Oracle attachments; qualified Pro does not fall back to it automatically.
...
Use genuine Web Multi-GPT only when independent parallel solvers are worth the latency and one merger can consume their file handoffs. It does not replace DevSpace evidence authority and must not increase calling-agent exploration.

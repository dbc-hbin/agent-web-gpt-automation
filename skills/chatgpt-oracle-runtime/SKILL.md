 ...
 If ChatGPT login expires in the isolated profile seed:

 1. **Close Main Chrome First**: Close all main Google Chrome windows completely before running recovery to release SQLite locks on the host Cookies database.
 2. **Recover Cookies from Main Chrome**:
    ```bash
    awgpt auth-recover --copy-profile "$HOME/.oracle/login-profiles/manual-login-..."
    ```
    *(Windows PowerShell: `awgpt auth-recover --copy-profile "$env:USERPROFILE\.oracle\login-profiles\manual-login-..."`)*
    - Copies and decrypts only `chatgpt.com` and `openai.com` cookies from the main Chrome profile into the seed.
    - Validates authentication against `/backend-api/me` in a throwaway test copy; if validation fails, automatically restores original seed files without leaving corrupt state.
 3. **Preflight Verification (No Submissions)**:
    ```bash
    awgpt auth-preflight --copy-profile "$HOME/.oracle/login-profiles/manual-login-..."
    ```
    - Verifies authentication and DOM presence (composer, model picker, thinking tier) without typing or submitting a prompt.
 4. **Manual Re-login (Fallback)**:
    If cookies cannot be recovered from Chrome or the session is fully invalidated:
    ```bash
    awgpt doctor --open-profile-login
    ```
    Complete manual login in the isolated browser window that opens, then close the browser.

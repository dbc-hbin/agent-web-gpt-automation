...
_AUTOMATION_ROOT = Path(__file__).resolve().parents[3]
if str(_AUTOMATION_ROOT / "bin") not in sys.path:
    sys.path.insert(0, str(_AUTOMATION_ROOT / "bin"))
...
def review_owner_password_interactive(
    *,
    ...
    output_fn("It will not be written to automation logs, receipts, manifests, or shell history.")
    return {
        "ok": True,
        "changed": changed,
...
def windows_bootstrap_watchdog_command(codex_home: Path | None = None) -> str:
    root = (codex_home or resolve_home()).resolve()

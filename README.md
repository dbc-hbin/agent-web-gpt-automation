 ...
 이 `python` 브랜치는 검증된 안정 Python 1.0.0 구현입니다. 기본 Node.js/TypeScript 구현(`awgpt`)은
 [`main`](https://github.com/dbc-hbin/agent-web-gpt-automation/tree/main) 브랜치에서 관리합니다.
 ...
 웹 GPT에 노출합니다. 저장소의 CLI와 lifecycle 설치기는 호출 에이전트에
 종속되지 않습니다. Codex 통합도 번들로 제공되며, 일반 웹 ChatGPT 에이전트도
 ...
 기존 Codex 설치는 `~/.codex`를 그대로 사용합니다. 새 비-Codex
 호스트는 `AGENT_WEB_GPT_HOME` 또는 `--agent-home`으로 중립 홈을 선택하며,
 아무 기존 홈도 없으면 `~/.agent-web-gpt`가 기본입니다. 설치기는 선택된 홈의
 `receipts`에 영수증을 남깁니다. Codex 통합을 사용한다면 설치 후 Codex를
 재시작하세요.
 ...
 1. **고정 공개 경로 설정** — Tailscale CLI + Funnel이 권장·자동화 경로입니다.
    Cloudflare, ngrok, custom HTTPS proxy는 사용자가 직접 구성·검증합니다.
 ...
 | [문서 인덱스](docs/README.md) | [전역 라우팅](docs/GLOBAL_CHATGPT_ROUTING.md) | [변경 기록](docs/CHANGELOG.md) | [버전 정책](docs/VERSIONING.md) |
 | [기여 가이드](CONTRIBUTING.md) | [보안 정책](SECURITY.md) | | [라이선스](LICENSE) |

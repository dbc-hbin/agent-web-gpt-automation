...
`main` is the stable Python implementation validated through installation and
skill dogfooding. The TypeScript implementation lives on the separate
[`ts`](https://github.com/dbc-hbin/agent-web-gpt-automation/tree/ts) branch.
...
approved by the user. The checked-in CLI and lifecycle installer are
caller-neutral, with a bundled Codex integration. A regular web ChatGPT agent
can inspect or execute against the exact approved root through DevSpace.
...
Existing Codex installs continue to use `~/.codex`. A fresh non-Codex host
selects a neutral home with `AGENT_WEB_GPT_HOME` or `--agent-home`; without an
existing home the default is `~/.agent-web-gpt`. Receipts are written under the
selected home. Restart Codex only when using the Codex integration.
...
for the recommended flow and the boundaries of manually managed providers.

1. **Configure a stable public route** — Tailscale CLI + Funnel is the
   recommended automated path. Configure and validate Cloudflare, ngrok, or a
   custom HTTPS proxy manually.
...
```powershell
python bin\chatgpt_oracle_dispatch.py `
  --mode orchestrator `
...
```bash
python3 bin/chatgpt_oracle_dispatch.py \
  --mode orchestrator \
  --project-root "$PWD" \
...
| [Documentation index](docs/README.md) | [Global Routing](docs/GLOBAL_CHATGPT_ROUTING.md) | [Changelog](docs/CHANGELOG.md) | [Versioning](docs/VERSIONING.md) |

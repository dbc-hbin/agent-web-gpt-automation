...
* **ProfileManager** creates a throwaway browser profile with `--copy-profile`
  from a manually signed-in seed. Normal runs never copy cookies or credentials
  back to the seed; recovery may reopen only the persisted conversation URL.
  The explicit `awgpt auth-recover` repair command is the sole exception: it
  imports only ChatGPT/OpenAI cookie rows and matching Chrome key metadata,
  validates login in a throwaway copy, and restores the seed if validation
  fails.

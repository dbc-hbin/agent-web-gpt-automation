...
The default registered app name is `codex`. To use another display name, set
`AGENT_WEB_GPT_APP_NAME` or store the identical name in
`%AGENT_WEB_GPT_HOME%\chatgpt-workspace.json`. `CODEX_CHATGPT_APP_NAME` and
`CODEX_HOME` remain compatibility aliases for existing installations.
...
mirror at `%AGENT_WEB_GPT_HOME%\config\codexpro-devspace-bootstrap.json`, retries while
Tailscale is still settling, and writes monthly logs under
`%AGENT_WEB_GPT_HOME%\logs\codexpro-devspace`. On Windows, `setup --apply` registers and
